...
pop = { colors.lightGray, colors.black }
...
function openFile(type) --draws the popup
    if type == "execute" then --Asks how the user wants to execute the seleceted file.
        ...
        local buttonSize = (w-18)/3 --Button distance
...
function drawList(typeList) --draws the current List
    if typeList == "complete" then
        ...
        for i=1,5,1 do
            ...
            elseif i == 3 then --categorie for the date(which is currently not yet supported, as CC:T ([1.15.2]1.87) is awaited) 
        ...
        for i=1,#items,1 do
            ...
            if items[i][3] == "-" then --Change's the current color for the unmarked item
                explorer.setBackgroundColor(normalItem[1])
                explorer.setTextColor(normalItem[2])
            elseif items[i][3] == "+" then --Change's the current color for the marked item
            ...
            if #items[i][2] > categorys-1 then
                explorer.write("  " .. string.sub(items[i][2], 1, categorys-6) .. "." .. string.sub(items[i][2], #items[i][2]-3)) --Here shortened
            else
                explorer.write("  " .. items[i][2]) --Here normally
            end

            --Show's here the size of the file
            explorer.setCursorPos(categorys+3, i+2+scroll)
            if fs.isDir(string.gsub(path, "\\", "/") .. items[i][2]) then --Writes only <DIR> if this is a folder
                dirsCount = dirsCount + 1
                explorer.write("<DIR>");
            else
                filesCount = filesCount + 1
                if not (items[i][2] == "..") then --Writes here the size of the file (also compressed to the desired size)
    ...
    elseif typeList == "lastItem" then --Redraw's the only Line who is changed in the list
        drawList("complete") --Serves only as placeholder
    end
